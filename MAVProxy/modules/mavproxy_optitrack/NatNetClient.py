...
class NatNetClient:
    # Originals "10.1.1.150" for server and "239.255.42.99" for multicast
    def __init__( self, serverIP = "192.168.0.101", multicastIP = "239.255.42.99"):
        print("Initialized modified OptiTrack module")
        ...
        # Set this to a callback method of your choice to receive per-rigid-body
        # data at each frame.
    ...
    # FUNCTIONS TO INTERFACE W/ mavproxy_optitrack
    def set_client_address(self, newIP):
        self.clientIPAddress = newIP

    def set_server_address(self, newIP):
        self.serverIPAddress = newIP
    ...
    def sendCommand( self, command, commandStr, socket, address ):
        ...
        #print(socket.sendto( data, address ))
    ...
    def run( self ):
        ...
        dataThread = th.Thread( target = self.__dataThreadFunction, args = (self.dataSocket, ))
        dataThread.start()

        # Create a separate thread for receiving command packets
        commandThread = th.Thread( target = self.__dataThreadFunction, args = (self.commandSocket, ))
        commandThread.start()

        self.sendCommand( self.NAT_REQUEST_MODELDEF, "", self.commandSocket, (self.serverIPAddress, self.commandPort) )


class NatNetClient_alt:
    # print_level = 0 off
    # print_level = 1 on
    ...
    def __init__( self ):
        ...
        # NatNet stream version serveris capable of. This will be updated during
        # initialization only.
        self.__nat_net_stream_version_server = [0,0,0,0]

        # NatNet stream version. This will be updated to the actual version the
        # server is using during runtime.
        self.__nat_net_requested_version = [0,0,0,0]

        # server stream version. This will be updated to the actual version the
        # server is using during initialization.
    ...
    def __unpack_skeleton( self, data, major, minor):
        ...
        for rb_num in range( 0, rigid_body_count ):
            offset_tmp, rigid_body = self.__unpack_rigid_body( data[offset:],
                major, minor, rb_num )
